import subprocess, json, pathlib, time, uuid, tempfile, urllib.request, hashlib, socket, os
...
try:
 ...
 run(['docker','run','-d','--rm','--name',frontend,'--network',net,'-p',f'127.0.0.1:{port}:8080','day-trader-rc-frontend:node24']);created.append(('container',frontend))
 routes=['/','/login','/portfolio','/activity','/research','/analytics','/robotrader','/trading-system','/stock/AAPL'];routing=[]
 for route in routes:
  for attempt in range(60):
   try:
    with http.open(f'http://127.0.0.1:{port}{route}',timeout=2) as response:body=response.read();status=response.status
    break
   except Exception:
    if attempt==59:raise
    time.sleep(.25)
