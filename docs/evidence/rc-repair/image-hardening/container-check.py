...
out=root/'docs/evidence/rc-repair/image-hardening';out.mkdir(parents=True,exist_ok=True)
...
server=None
tlsdir=tempfile.TemporaryDirectory(prefix='day-trader-runtime-tls-')
run_tls=None
...
try:
 ...
 certdir=pathlib.Path(tlsdir.name)
 run(['openssl','req','-x509','-newkey','rsa:2048','-nodes','-keyout',str(certdir/'key.pem'),'-out',str(certdir/'cert.pem'),'-days','1','-subj','/CN=localhost','-addext','subjectAltName=DNS:localhost'])
 os.chmod(certdir,0o755);os.chmod(certdir/'key.pem',0o644)
 run(['docker','run','-d','--rm','--name',backend,'--network',net,'--env-file',str(envpath),'--mount',f'type=bind,src={certdir},dst=/local-tls,readonly',os.environ['RC_BACKEND_IMAGE']]);created.append(('container',backend));envpath.unlink()
 for attempt in range(60):
 ...
 tlsprobe="""const assert=require('node:assert/strict'),fs=require('node:fs'),https=require('node:https'),tls=require('node:tls');const cert=fs.readFileSync('/local-tls/cert.pem');assert.ok(tls.getCACertificates('extra').length>100);const server=https.createServer({key:fs.readFileSync('/local-tls/key.pem'),cert},(q,r)=>r.end('verified local TLS'));server.listen(0,'127.0.0.1',()=>{https.get({hostname:'localhost',family:4,port:server.address().port,ca:cert},r=>{let b='';r.on('data',c=>b+=c);r.on('end',()=>{assert.equal(b,'verified local TLS');console.log(JSON.stringify({localTlsVerified:true,extraCaCount:tls.getCACertificates('extra').length,node:process.version}));server.close()})}).on('error',e=>{console.error(e);server.close();process.exitCode=1})});"""
 run(['docker','exec',backend,'node','-e',tlsprobe])
 run(['docker','exec',backend,'node','-e',"const a=require('node:assert/strict');a.equal(process.env.ROBO_SCHEDULER_DISABLED,'true');a.equal(process.env.ROBOTRADER_WORKER_DISABLED,'true');a.equal(require('fs').existsSync('/usr/local/bin/npm'),false);console.log('scheduler test mode disabled; npm absent')"])

 run(['docker','run','-d','--rm','--name',frontend,'--network',net,os.environ['RC_FRONTEND_IMAGE']]);created.append(('container',frontend))
 ...
 for image in [os.environ['RC_BACKEND_IMAGE'],os.environ['RC_FRONTEND_IMAGE']]:
 ...
 waiter=subprocess.Popen(['docker','wait',backend],stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
 run(['docker','stop','--time','5',backend]);waitout,waiterr=waiter.communicate(timeout=15)
 report['shutdown']={'exit':waitout.strip(),'stderr':waiterr};assert waitout.strip() in ['0','143']
 created.remove(('container',backend));report['ok']=True
