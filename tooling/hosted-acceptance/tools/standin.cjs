...
const server=https.createServer({key:fs.readFileSync('/certs/key.pem'),cert:fs.readFileSync('/certs/cert.pem')},(req,res)=>{
 transportObservation.http(req,res);
 require('./security-probe.cjs').observeBoundary(req,res,'backend');
...
});const transportObservation=require('./transport-observation.cjs').observeStandin(server);server.listen(443,'0.0.0.0');
