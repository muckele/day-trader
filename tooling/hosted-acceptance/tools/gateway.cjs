'use strict';
const fs=require('fs'),http=require('http'),net=require('net');const c=require('./config.cjs').load(),{parseRequest,limitFor}=require('./policy.cjs'),{forward}=require('./transport.cjs');
...
const server=http.createServer({maxHeaderSize:16384,requestTimeout:15000},async(req,res)=>{
 const d=parseRequest({method:req.method,target:req.url,rawHeaders:req.rawHeaders},assets);if(!d){log({event:'denied'});res.writeHead(403);return res.end();}
 const key=state.phase+':'+d.id,count=state.counts[key]||0;
 // Login/logout ceiling is GLOBAL, including uncertain transport outcomes.
 const authCount=state.counts['global:'+d.id]||0;
 if(count>=limitFor(d.id,state.phase)||(['login','logout'].includes(d.id)&&authCount>=1)||state.paperReserved+d.cost>c.paperMaximum){log({event:'budget-denied',route:d.id});res.writeHead(429);return res.end();}
 state.counts[key]=count+1;state.counts['global:'+d.id]=authCount+1;state.paperReserved+=d.cost;state.attempts++;persist();
 const id=state.attempts;let body=Buffer.alloc(0);try{
  ...
  const r=await forward(c,d,body,up=>{if([300,301,302,303,305,307,308].includes(up.statusCode)){up.resume();log({id,event:'redirect-denied',route:d.id,status:up.statusCode});res.writeHead(502);return res.end();}log({id,event:'forward',phase:state.phase,route:d.id,method:d.method,status:up.statusCode,paperReserved:state.paperReserved});res.writeHead(up.statusCode,up.headers);up.pipe(res);});r.on('error',()=>{log({id,event:'upstream-error',route:d.id});if(!res.headersSent)res.writeHead(502);res.end();});
