...
sudo env GITHUB_ACTIONS=true RUNNER_ENVIRONMENT=github-hosted PILOT_STATE="$PILOT_STATE" GITHUB_RUN_ATTEMPT="$GITHUB_RUN_ATTEMPT" GITHUB_REF="$GITHUB_REF" PILOT_REPOSITORY_VISIBILITY="$PILOT_REPOSITORY_VISIBILITY" PILOT_APPLICATION_SOURCE="$PILOT_APPLICATION_SOURCE" PILOT_PROFILE="$PILOT_PROFILE" PILOT_QUALIFICATION="$PILOT_QUALIFICATION" PILOT_DIAGNOSTIC_ONLY="$PILOT_DIAGNOSTIC_ONLY" PILOT_SECURITY_PROBE_DIAGNOSTIC="$PILOT_SECURITY_PROBE_DIAGNOSTIC" python3 tooling/hosted-acceptance/run.py
