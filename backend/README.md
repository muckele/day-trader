...
- `ROBO_MIN_MINUTES_BETWEEN_SYMBOL_EXECUTIONS` (default `1440`; symbol-level cooldown window; set `0` only to intentionally allow repeated same-symbol trades)
- `ROBO_MAX_EXECUTIONS_PER_DAY` (default uses Robo settings `maxTradesPerDay`, currently `3`; set `0` only to intentionally allow unlimited legacy executions)
