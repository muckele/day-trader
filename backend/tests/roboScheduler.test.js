...
test('startRoboScheduler runs scheduler tick and retention cleanup', async t => {
  ...
  t.mock.method(roboTraderWorker, 'cleanupRoboAuditLogs', async args => {
    auditCleanupCalls.push(args);
    return { deletedCount: 0, retentionDays: 7 };
  });
  ...
  try {
    ...
    assert.ok(auditCleanupCalls.length >= 1);
    assert.equal(auditCleanupCalls[0].olderThanDays, 7);
  } finally {
...
test('startRoboScheduler defaults to Phase 1 worker without running legacy scheduler', async t => {
  ...
  t.mock.method(roboTraderWorker, 'cleanupRoboAuditLogs', async () => ({ deletedCount: 0, retentionDays: 7 }));
