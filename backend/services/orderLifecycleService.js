...
function createOrderLifecycle({broker,ownerId,expectedAccountId,loadSettings=async()=>null,now=()=>new Date(),afterBrokerSubmit,afterReconcile,beforeEntry,strictRisk=false,uncertaintyMs=300000}={}) {
 ...
 async function reserve(i,amount,settings,a,s){
  ...
  await adjust(i,amount,0,s);i.reservedCents+=amount;
 }
 async function submitUnlocked({userId,idempotencyKey,origin='manual',orderInput,beforeSubmit,beforeBrokerWrite}={}){
  ...
  const positions=broker.getPositions?await broker.getPositions():[];
  let i;
  try{i=await transaction(async s=>{
 ...
 async function reconcile({intentId}){
  ...
  i=await owned(intentId);if(['submitting','submission_uncertain','replace_pending'].includes(i.status)&&now()-i.uncertainSince>=uncertaintyMs){i=await localTransition(i._id,'reconciliation_required',null,['submitting','submission_uncertain','replace_pending']);}
  return result(i);
 }
 async function cancel({intentId}){
  ...
  await localTransition(i._id,'cancel_pending');await broker.cancelOrder(order.externalOrderId);return reconcile({intentId});
 }
 async function replace({intentId,changes,idempotencyKey}){
  ...
  const a=await account(),settings=await loadSettings(ownerId);if(!settings||settings.mode!=='paper')throw fail('Paper settings required.');if(/^robo/i.test(i.origin)&&(!settings.isEnabled||settings.pausedUntil&&new Date(settings.pausedUntil)>now()))throw fail('Automated entries disabled or paused.');if(broker.getClock){const clock=await broker.getClock();if(!clock.is_open||!clock.timestamp||Math.abs(now()-new Date(clock.timestamp))>60000)throw fail('Fresh open market clock required.');}const b=await BrokerOrder.findOne({...scope,intentId}).sort({createdAt:-1});if(!b)throw fail('Confirmed broker order required.');
