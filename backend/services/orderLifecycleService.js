...
function assertFreshOpenClock(clock,nowMs){
 const freshness=evaluateBrokerClock(clock,{nowMs});
 if(!freshness.valid||clock.is_open!==true)throw Object.assign(fail('Fresh open market clock required.',freshness.valid?'MARKET_CLOSED':'FRESH_CLOCK_REQUIRED'),{clockFreshness:freshness});
}
...
function createOrderLifecycle({broker,ownerId,expectedAccountId,loadSettings=async()=>null,now=()=>new Date(),afterBrokerSubmit,afterReconcile,beforeEntry,beforeAdmission,strictRisk=false,uncertaintyMs=300000}={}) {
 ...
 async function submitUnlocked({userId,idempotencyKey,origin='manual',orderInput,beforeSubmit,beforeBrokerWrite,dispatchContext}={}, coordinatedReduction=false){
  ...
  const normalized=coordinatedReduction?require('./orderLifecycleFinancial').normalizeReducingOrder(orderInput):normalizeOrder(orderInput),fingerprint=hash(normalized);const key={...scope,environment:'paper',idempotencyKey};
  ...
  if((normalized.side==='buy'||coordinatedReduction)&&broker.getClock){const clock=await broker.getClock();assertFreshOpenClock(clock,+now());}
  ...
  if(coordinatedReduction&&!Q.whole(normalized.qty)){const asset=await broker.getAsset?.(normalized.symbol);if(!asset||asset.fractionable!==true||asset.tradable!==true||asset.status!=='active'||asset.class!=='us_equity')throw fail('Fractional reduction requires a fractionable active equity.');}
  ...
  try{exposureSnapshot=strictRisk&&normalized.side==='buy'?await Exposure.capture({broker,accountId:expectedAccountId}):null;positions=exposureSnapshot?exposureSnapshot.positions:broker.getPositions?await broker.getPositions():[];i=await transaction(async s=>{
   ...
   else {const capacity=await lock(s);if(strictRisk)positions=await Exposure.reducingPositions({accountId:expectedAccountId,positions,cap:capacity,session:s});const position=positions.find(p=>p.symbol===normalized.symbol);const available=Q.units(position?.qty_available??position?.qty??0);const pending=await OrderIntent.find({...scope,symbol:normalized.symbol,side:'sell',status:{$nin:[...terminal]}}).session(s);const outstanding=pending.filter(p=>String(p._id)!==String(created._id)).reduce((n,p)=>n+Q.units(p.qty)-Q.units(p.filledQty),0n);if(Q.units(normalized.qty)+outstanding>available)throw fail('Exit exceeds available position.');}
  ...
  try{if(beforeSubmit)await beforeSubmit();await account();if(coordinatedReduction&&(!Q.whole(normalized.qty)||broker.getClock)){const c=await broker.getClock?.();assertFreshOpenClock(c,+now());}if(normalized.side==='buy'){const currentSettings=await loadSettings(userId);if(!currentSettings||currentSettings.mode!=='paper')throw fail('Paper settings unavailable.');for(const field of ['dailyLimit','weeklyLimit','monthlyLimit'])cents(currentSettings[field]);if(strictRisk&&normalized.ceilingCents>cents(currentSettings.maxTradeAmount))throw fail('Maximum trade amount changed.');if(/^robo/i.test(origin)&&(!currentSettings.isEnabled||currentSettings.pausedUntil&&new Date(currentSettings.pausedUntil)>now()))throw fail('Automated entries disabled or paused.');if(broker.getClock){const c=await broker.getClock();assertFreshOpenClock(c,+now());}}if(/^robo/i.test(origin)&&beforeEntry)await beforeEntry();}catch(error){await transaction(async s=>{await lock(s);const blocked=await owned(i._id,s);if(blocked.status==='reserved'){await adjust(blocked,-blocked.reservedCents,0,s);blocked.reservedCents=0;blocked.status='rejected';blocked.rejectionReason=error.message;await blocked.save({session:s});await event(blocked,s);}});throw error;}
 ...
 async function ingest(id,raw){
  ...
  const i=await transaction(async s=>{
   ...
   const acknowledged=Boolean(b);
   if(!b)b=new BrokerOrder({...scope,intentId:intent._id,externalOrderId:String(raw.id),clientOrderId:raw.client_order_id,broker:'alpaca',origin:intent.origin,symbol:intent.symbol,side:intent.side,qty:Q.persist(raw.qty??intent.qty),orderType:intent.orderType});
   const q=Q.units(raw.filled_qty??0),ordered=Q.units(raw.qty??intent.qty);
   const requested=Q.units(acknowledged?b.qty:raw.client_order_id===intent.replacement?.clientOrderId?intent.replacement.orderInput.qty:intent.qty);
   if(q>ordered||ordered>requested)throw fail('Broker cumulative quantity exceeds ordered quantity.');
   const total=q?fillNotionalCents(Q.format(q),raw.filled_avg_price):0;
   if(q>Q.units(b.filledQty)){b.filledQty=Q.persist(Q.format(q));b.filledNotionalCents=total;}
   const deltaQ=q>Q.units(intent.filledQty)?q-Q.units(intent.filledQty):0n;
   const deltaSpent=deltaQ?total-intent.filledNotionalCents:0;
   if(deltaSpent<0)throw fail('Broker cumulative notional regressed.');
   if(deltaQ){await Fill.create([{...scope,broker:'alpaca',intentId:intent._id,brokerOrderId:b._id,externalOrderId:String(raw.id),cumulativeQty:Q.persist(Q.format(q)),symbol:intent.symbol,side:intent.side,qty:Q.persist(Q.format(deltaQ)),price:deltaSpent/Number(Q.format(deltaQ))/100,notional:deltaSpent/100,notionalCents:deltaSpent}],{session:s});intent.filledQty=Q.persist(Q.format(q));intent.filledNotionalCents=total;await Exposure.invalidate(expectedAccountId,s);}
 ...
 async function replace({intentId,changes,idempotencyKey}){
  ...
  if(!['acknowledged','partially_filled'].includes(i.status)||Q.compare(normalized.qty,i.filledQty)<0||normalized.side!==i.side||normalized.symbol!==i.symbol)throw fail('Order cannot be replaced in this state.');
  const a=await account(),settings=await loadSettings(ownerId);if(!settings||settings.mode!=='paper')throw fail('Paper settings required.');if(broker.getClock){const clock=await broker.getClock();assertFreshOpenClock(clock,+now());}const b=await BrokerOrder.findOne({...scope,intentId}).sort({createdAt:-1});if(!b)throw fail('Confirmed broker order required.');
  const clientOrderId=`mvp-r-${hash([i.clientOrderId,idempotencyKey]).slice(0,38)}`;
  await transaction(async s=>{const current=await owned(intentId,s);if(current.replacement||!['acknowledged','partially_filled'].includes(current.status)||Q.compare(normalized.qty,current.filledQty)<0)throw fail('Replacement state changed; reconcile first.');if(settings?.maxTradeAmount&&normalized.ceilingCents>cents(settings.maxTradeAmount))throw fail('Maximum trade amount exceeded.');const additional=Math.max(0,normalized.ceilingCents-current.filledNotionalCents-current.reservedCents);if(additional)await reserve(current,additional,settings,a,s);else await lock(s);current.replacement={idempotencyKey,fingerprint,clientOrderId,status:'pending',previousBrokerOrderId:b.externalOrderId,previousStatus:current.status,additionalCents:additional,orderInput:normalized};current.status='replace_pending';current.uncertainSince=now();await current.save({session:s});await event(current,s);});
  let brokerResponded=false;
 ...
 const closeService=require('./positionCloseService').createPositionCloseService({broker,ownerId,expectedAccountId,submit:request=>submitUnlocked(request,true),reconcile,cancel,beforeAdmission,now});
