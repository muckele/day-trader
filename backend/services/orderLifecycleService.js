...
function createOrderLifecycle({broker,ownerId,expectedAccountId,loadSettings=async()=>null,now=()=>new Date(),afterBrokerSubmit,afterReconcile,beforeEntry,beforeAdmission,strictRisk=false,uncertaintyMs=300000}={}) {
 ...
 async function submitUnlocked({userId,idempotencyKey,origin='manual',orderInput,beforeSubmit,beforeBrokerWrite,dispatchContext}={}, coordinatedReduction=false){
  ...
  if((normalized.side==='buy'||coordinatedReduction)&&broker.getClock){const clock=await broker.getClock();if(!clock.is_open||!clock.timestamp||Math.abs(now()-new Date(clock.timestamp))>60000)throw fail('Fresh open market clock required.');}
